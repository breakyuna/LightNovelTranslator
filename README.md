...
- 按模型上下文预算和自然段切分长章节，输出截断时最多续写两次，并保留已完成分块。
...
- 导出 TXT 或 EPUB；每次真实 LLM 请求独立记录 token、费用、耗时、尝试次数和失败原因。
- 翻译运行与分块状态持久化，支持暂停、取消、应用重启后的中断标记和已完成分块复用。
- 段落关系以稳定 Segment ID 持久化，编辑和段落重译按 Segment 定位；EPUB 解析支持 InputStream/File 流式入口，图片直接落盘。
